...
impl ConfigManager {
    ...
    pub fn insert_learned(&self, profile: &str, pinyin: &str, entries: &[(String, u32)]) {
        ...
        // Atomically update short-term memory
        let short = self.learned_words.rcu(|current| {
            ...
            if let Some(profile_data) = clone.get_mut(profile) {
        ...
        // Atomically promote eligible entries to long-term memory
        self.long_term_words.rcu(|current| {
            let mut long = (**current).clone();
            let mut changed = false;
            if let Some(profile_data) = short.get(profile) {
                for (py, words) in profile_data.iter() {
            ...
            if changed { Arc::new(long) } else { current.clone() }
        });
    ...
    pub fn insert_usage_order(&self, profile: &str, word: &str) {
        let mru = self.master_config.input.mru_length as usize;
        self.user_order.rcu(|current| {
            let mut clone = (**current).clone();
            let entries = clone.entry(profile.to_string()).or_default();
            entries.retain(|w| w != word);
            entries.insert(0, word.to_string());
            entries.truncate(mru);
            Arc::new(clone)
        });
    }
