    ...
    if let Some(ref tx) = gui_tx {
        ...
        if let Ok(mut state) = app_state.lock() {
            ...
            if !buffer_empty || !state.status_text.contains("...") {
                state.status_text = if chinese_enabled {
                    short_display
                } else {
                    "英".into()
                };
            ...
            if buffer_empty || !chinese_enabled {
                ...
                let mut pinyin = if best_segmentation.is_empty() {
                    buffer
                } else {
                    best_segmentation.join(" ")
                };
                if nav_mode {
                ...
                if !aux_filter.is_empty() {
                    ...
                    for (i, c) in aux_filter.chars().enumerate() {
                        if i == 0 {
                            display_aux.push(c.to_ascii_uppercase());
                        } else {
                            display_aux.push(c.to_ascii_lowercase());
                        }
                    }
                ...
                state.candidates = candidates
                    .iter()
                    .map(|c| crate::ui::DisplayCandidate {
...
fn get_system_cursor_pos() -> Option<(i32, i32)> {
    unsafe {
        ...
        use windows::Win32::UI::WindowsAndMessaging::{
            GetCaretPos, GetForegroundWindow, GetGUIThreadInfo, GUITHREADINFO,
        };
                ...
                std::thread::spawn(move || loop {
                    unsafe {
                        ...
                        if connect_res.is_ok()
                            || connect_res.err().map_or(false, |e| {
                                e.code()
                            ...
                            })
                        {
                            ...
                            std::thread::spawn(move || {
                                handle_client(h, pi, gi, ti, ci, ai);
                                let _ = CloseHandle(h);
                            });
                        } else {
                            let _ = CloseHandle(h);
...
fn is_hk_match(
    ...
    if req_ctrl != ctrl || req_alt != alt || req_shift != shift {
        return false;
    }
    ...
    loop {
        ...
        if msg_type == 1 && bytes_read >= 14 {
            ...
            if (x != 0 || y != 0) && gui_tx.is_some() {
                let _ = gui_tx
                    .as_ref()
                    .expect("gui_tx is some")
                    .send(crate::ui::GuiEvent::MoveTo { x, y });
            }
        }
        ...
        let (enable_tab, enable_ctrl_space, switch_key) = {
            if let Ok(c) = config.read() {
                (
                    c.hotkeys.enable_tab_toggle,
                    c.hotkeys.enable_ctrl_space_toggle,
                    c.hotkeys.switch_language.key.clone(),
                )
            } else {
                (false, false, "".into())
            }
        ...
        if (ctrl || alt) && !is_ctrl_space_match {
            let buffer_empty = processor
                .lock()
                .map(|p| p.session.buffer.is_empty())
                .unwrap_or(true);
            if buffer_empty {
        ...
        if let Some(key) = key {
            ...
            if msg_type == 1 || msg_type == 3 {
                if let Ok(mut p) = processor.lock() {
                    let action =
                        p.handle_key(key, if msg_type == 1 { 1 } else { 0 }, shift, ctrl, alt);
                    drop(p);
                    match action {
                        Action::Emit(txt) => {
                            response.push(1);
                ...
                if let Ok(p) = processor.lock() {
                    ...
                    let is_punct = match key_code {
                        0x20 | 0xC0 | 0xBD | 0xBB | 0xDB | 0xDD | 0xDC | 0xBA | 0xDE | 0xBC
                        | 0xBE | 0xBF => true,
                        0x30..=0x39 if shift => true,
                        _ => false,
                    };
                    if p.chinese_enabled
                        && (!p.session.buffer.is_empty() || is_letter || is_punct || is_special)
                    {
                        response.push(2);
                    } else {
                        response.push(0);
