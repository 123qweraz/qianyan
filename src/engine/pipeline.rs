        ...
        while pos < input.len() {
            ...
            if !matched {
                let ch = input[pos..]
                    .chars()
                    .next()
                    .expect("Should have at least one character remaining");
...
impl Translator for TableTranslator {
    fn translate(
        ...
        // 检查缓存是否可以复用（增量搜索优化）
        {
            let cached = self.cached_candidates.read().expect("Lock poisoned");
            let (last_q, last_time) = &*self.last_query.read().expect("Lock poisoned");
        ...
        // 更新缓存
        {
            let mut last_q = self.last_query.write().expect("Lock poisoned");
            *last_q = (query, std::time::Instant::now());
            let mut cached = self.cached_candidates.write().expect("Lock poisoned");
