...
这三个设置其实可以合并成一个设置才对，叫候选词UI显示模式， 可变成选择框， 提供四个选项， 系统桌面通知， 输入法slint，无， 同时显示，

拼音设置优化，应该把所有算法都开放给用户选择，精确匹配，前缀匹配，简拼算法，调频算法，纠错算法，自动造词算法，自动组句算法的开关，参数都开放给用户，然用户可以在网页的拼音设置进行设置
...
加个中英文切换通知，可以在按capslock切换中英文时，用桌面通知进行通知，会显示一个简洁的中， 或者英，随后立马消失，用户可以在网页上设置是否打开状态切换通知，不要和通知候选词混淆哦
